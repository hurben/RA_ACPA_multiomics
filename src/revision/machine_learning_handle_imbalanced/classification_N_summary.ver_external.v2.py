...
if __name__ == "__main__":
	...
	output_txt.write('\tFNR_average\tFNR_stdev\tNPV_average\tNPV_stdev\tAUC_average\tAUC_stdev\n')
	...
	for split_type in list(split_type_dict.keys()):
		...
		for feature_size in split_list:
			...
			for j in range(100):
				...
				y_pred_proba = clf.predict_proba(X_test_balanced)[:, 1]
				auc = roc_auc_score(y_test_balanced, y_pred_proba)
				resample_auc_list.append(auc)
...
#			print (resample_acc_list)
			...
			output_txt.write('\t%s\t%s' % (statistics.mean(resample_auc_list), statistics.stdev(resample_auc_list)))
			output_txt.write('\n')
