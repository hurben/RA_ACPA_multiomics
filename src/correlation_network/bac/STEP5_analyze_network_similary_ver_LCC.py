...
def find_subgraphs(edge_list):
	...
	sub_graphs = nx.connected_components(data_graph) #Get connected components
	sub_graphs_list = list(sub_graphs)
	largest_cc = max(nx.connected_components(data_graph), key=len)
	LCC_node_list = list(largest_cc)

	return data_graph, sub_graphs_list, LCC_node_list
...
def write_output(output_name, output_type, input_list):
	...
	data_graph, sub_graphs_list, LCC_node_list = find_subgraphs(input_list) 
	data_graph.to_undirected()

	#Make LCC_graph: This process is necessary to avoid duplicate edges.
	LCC_graph = nx.Graph()
	for node in LCC_node_list:
		edge_list = data_graph.edges(node)
		LCC_graph.add_edges_from(edge_list)
	LCC_edge_list = list(LCC_graph.edges())
	
	output_file = '%s.%s.topology.LCC.tsv' % (output_name, output_type)
	output_txt = open(output_file,'w')
	output_txt.write('target\tsource\n')

	#STEP3: write sub topology
	for edge in LCC_edge_list:
		source = edge[0]
		target = edge[1]
		output_txt.write('%s\t%s\n' % (source, target))
	output_txt.close()
	...
	output_file = '%s.%s.topology.LCC.log' % (output_name, output_type)
	output_txt = open(output_file,'w')
	...
	output_txt.write('LCC node size: %s\n' % len(LCC_graph.nodes()))
	output_txt.write('LCC edge size: %s\n' % len(LCC_graph.edges()))
