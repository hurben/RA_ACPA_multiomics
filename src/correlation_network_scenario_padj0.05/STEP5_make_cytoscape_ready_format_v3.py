# STEP5_make_cytoscape_ready_format_v3.py         23.02.10
...
if __name__ == "__main__":
	...
	#data_dict = #topology_dict[source, target, topology_type] = [rho, padj]

	output_txt = open(output_file,'w')
	output_txt.write('source\ttarget\tinfo_type\tedge_class\tpval\trho\tcontrol_pval\tcontrol_rho\n')
	...
	for edge in edge_list:
		...
		try: rho = data_dict[source, target, condition][0]
		except KeyError: rho = data_dict[target, source, condition][0]

		edge_class = "unknown"
		if rho > 0:
			edge_class = "%s_rho_pos" % condition
		if rho < 0:
			edge_class = "%s_rho_neg" % condition

		output_txt.write('%s\t%s\t%s\t%s' % (source,target,condition, edge_class))
		...
		output_txt.write('%s\t%s\t%s\t%s' % (source, target, condition, edge_class)) #writing target, source reversed for visualization purpose
